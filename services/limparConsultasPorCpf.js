...
async function deleteUsersKycByCpf(dbClient, { usersKycDatabase, cpfDigits }) {
  return dbClient.query(
    'delete from users_kyc where "database" = $1 and document = $2',
    [usersKycDatabase, cpfDigits]
  );
}

export async function limparConsultasPorCpf(cpf) {
  const cpfDigits = normalizeCpf(cpf);
  if (cpfDigits.length !== 11) {
    throw new Error(`limparConsultasPorCpf: CPF inválido: ${cpf}`);
  }
  ...
  try {
    ...
    const delUsersKyc = await deleteUsersKycByCpf(documentClient, { usersKycDatabase, cpfDigits });
